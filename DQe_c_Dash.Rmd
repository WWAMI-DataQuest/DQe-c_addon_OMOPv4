...
path2 = "..." #set the path to where DQe-c report files from individual databases are saved
...
lateDQM <- DQ_M %>% group_by(organization) %>% summarise(maxdate = max(test_date))
lateDQM$load <- paste0(lateDQM$organization,lateDQM$maxdate)
